...
/* number of bytes we read per chunk *//*
...
typedef void(*h264_decoder_callback)(AVFrame* frame, AVPacket* pkt, void* user);         */
...
/* d'tor, cleans up the allocated objects and closes the codec context *//*
    ...
    bool load(std::string filepath, float fps = 0.0f);                                     */
/* load a video file which is encoded with x264 *//*

    bool readFrame();                                                                      */
/* read a frame if necessary *//*
